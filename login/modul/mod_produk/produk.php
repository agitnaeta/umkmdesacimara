...
switch($_GET[act]){
  ...
  default:
    ...
    // echo "SELECT * FROM produk $add_sql ORDER BY id_produk DESC LIMIT $posisi,$batas";

    $tampil = mysql_query("SELECT * FROM produk $add_sql ORDER BY id_produk DESC LIMIT $posisi,$batas");
   ...
   $add_sql = get_kategori();
    echo "<h2>Tambah Produk</h2>
          ...
          <select name='kategori'>
            ...
            $tampil=mysql_query("SELECT * FROM kategori $add_sql  ORDER BY nama_kategori");
            while($r=mysql_fetch_array($tampil)){
